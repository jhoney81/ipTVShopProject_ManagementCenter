...
public class ManagementCenter {
    ...
    public void onPostPersist() {
        ...
        }else if("ASORDED".equals(this.getStatus())) {

            AsAccepted asAccepted = new AsAccepted();

            asAccepted.setId(this.getId());
            asAccepted.setInstallationAddress(this.getInstallationAddress());
            asAccepted.setOrderId(this.getId());
            asAccepted.setStatus(this.getStatus());
            asAccepted.setEngineerId(this.getEngineerId());
            asAccepted.setEngineerName(this.getEngineerName());

            BeanUtils.copyProperties(this, asAccepted);
    ...
    public void onPostUpdate() {
        ...
        //AS 완료 통보 후 AS Completed 이벤트 호출.
        else if (this.getStatus().equals("VISITATIONCOMPLETED")) {

            System.out.println("22222222222222222" + this.getStatus());

            AsCompleted asCompleted = new AsCompleted();

            asCompleted.setEngineerId(this.getEngineerId());
            asCompleted.setEngineerName(this.getEngineerName());
            asCompleted.setId(this.getId());
            asCompleted.setInstallationAddress(this.getInstallationAddress());
            asCompleted.setOrderId(this.orderId);
            asCompleted.setStatus(this.getStatus());

            BeanUtils.copyProperties(this, asCompleted);
            asCompleted.publishAfterCommit();
