...
public class PolicyHandler{
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void wheneverAsOrdered_AsRequest(@Payload AsOrdered asOrdered){

        if(asOrdered.isMe()){

            ManagementCenter oa = new ManagementCenter();

            oa.setOrderId(asOrdered.getId());
            oa.setInstallationAddress(asOrdered.getInstallationAddress());
            oa.setId(asOrdered.getId());
            oa.setStatus("ASORDED");
            oa.setEngineerName("Engineer" + asOrdered.getId());
            oa.setEngineerId(asOrdered.getId() + 100);

            managementCenterRepository.save(oa);

            System.out.println("##### listener AsRequest : " + asOrdered.toJson());
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void wheneverVisitationCompleted_VisitationCompleteNotify(@Payload VisitationCompleted visitationCompleted){

        if(visitationCompleted.isMe()){
            System.out.println("##### listener VisitationCompleteNotify : " + visitationCompleted.toJson());
        }

        //방문 완료 통보 시 기존 Order ID의 AS 완료 상태(Completed) 저장.
        if(visitationCompleted.isMe()) {
            System.out.println("ORDERID" + visitationCompleted.getOrderId());
            System.out.println("GETID" + visitationCompleted.getId());
            try
            {
                System.out.println("##### listener VisitationCompleteNotify : " + visitationCompleted.toJson());
                managementCenterRepository.findById(visitationCompleted.getOrderId())
                        .ifPresent(
                                managementCenter -> {
                                    managementCenter.setStatus(visitationCompleted.getStatus());
                                    managementCenterRepository.save(managementCenter);
                                });
            }
            catch (Exception e){
